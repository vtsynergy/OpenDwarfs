...
void grunSyrTwoK(DATA_TYPE **a_a, DATA_TYPE **b_a, DATA_TYPE **c_a)
{
    int i, j, k, n, m;
...
#pragma omp for private(i,j) schedule(runtime)
        ...
        do{
            ...
            a = ctsar_reg_mem(s, *a_a, sizeof(DATA_TYPE)*N, N,
                    CTSAR_MEM_INPUT);
            b = ctsar_reg_mem(s, *b_a, sizeof(DATA_TYPE)*N, N,
                    CTSAR_MEM_INPUT);
...
void runSyrTwoK(DATA_TYPE **a, DATA_TYPE **b, DATA_TYPE **c)
{
    int i, j, k, n, m;
    DATA_TYPE alpha, beta;
...
int main(int argc, char** argv)
{
    ...
    DATA_TYPE **B = (DATA_TYPE **)ctsar_alloc_2d(sizeof(DATA_TYPE),N,M);
    ...
    for(int i=0; i < iters; i++)
    {
        ...
        if((env = getenv("OMP_CTSAR_FALLBACK")) && atoi(env)){
            ...
            ctsar_init(&s,N,CTSAR_START, CTSAR_DEV_CPU | CTSAR_DEV_GPU, NULL,NULL,NULL);
            grunSyrTwoK(A, B, C_outputFromGpu);
        }
        lt_end = rtclock();
